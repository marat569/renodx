...
float3 ApplyBlueCorrectionPre(float3 untonemapped_ap1) {
  ...
  float corrected_r = ((mad(0.061360642313957214f, b, mad(-4.540197551250458e-09f, g, (r * 0.9386394023895264f))) - r) * ue_bluecorrection) + r;
  float corrected_g = ((mad(0.169205904006958f, b, mad(0.8307942152023315f, g, (r * 6.775371730327606e-08f))) - g) * ue_bluecorrection) + g;
  float corrected_b = (mad(-2.3283064365386963e-10f, g, (r * -9.313225746154785e-10f)) * ue_bluecorrection) + b;
...
float3 ApplyBlueCorrectionPost(float3 tonemapped) {
  ...
  float _1149 = ((mad(-0.06537103652954102f, _1133, mad(1.451815478503704e-06f, _1132, (_1131 * 1.065374732017517f))) - _1131) * ue_bluecorrection) + _1131;
  float _1150 = ((mad(-0.20366770029067993f, _1133, mad(1.2036634683609009f, _1132, (_1131 * -2.57161445915699e-07f))) - _1132) * ue_bluecorrection) + _1132;
  float _1151 = ((mad(0.9999996423721313f, _1133, mad(2.0954757928848267e-08f, _1132, (_1131 * 1.862645149230957e-08f))) - _1133) * ue_bluecorrection) + _1133;
...
void ApplyFilmToneMapWithBlueCorrect(float untonemapped_r, float untonemapped_g, float untonemapped_b,
  ...
  if (RENODX_TONE_MAP_TYPE == 0.f) {  // Vanilla
    tonemapped_prebluecorrect_ap1 =
        unrealengine::filmtonemap::ApplyToneCurve(untonemapped_rrt_prebluecorrect_ap1, ue_filmslope, ue_filmtoe, ue_filmshoulder, ue_filmblackclip, ue_filmwhiteclip);
  } else if (RENODX_TONE_MAP_TYPE == 1.f) {
    tonemapped_prebluecorrect_ap1 =
        ApplyToneCurveExtendedWithHermite(untonemapped_rrt_prebluecorrect_ap1, ue_filmslope, ue_filmtoe, ue_filmshoulder, ue_filmblackclip, ue_filmwhiteclip);
  }
...
float3 LUTToneMap(float3 untonemapped, float rolloff_start = 0.5f, float output_max = 1.f) {
  ...
  float white_clip = ENODX_PEAK_WHITE_NITS / RENODX_DIFFUSE_WHITE_NITS;
...
float3 GamutDecompress(float3 color_linear, float gamut_compression_scale) {
  if (CUSTOM_LUT_GAMUT_RESTORATION == 0.f || gamut_compression_scale == 1.f) return color_linear;

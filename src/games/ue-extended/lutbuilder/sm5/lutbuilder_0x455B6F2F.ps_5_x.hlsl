...
void main(
  ...
  float4 lutweights[2] = { float4(asfloat(cb0[34].x), asfloat(cb0[35].x), 0.f, 0.f), float4(0.f, 0.f, 0.f, 0.f) };
  cb_config.ue_lutweights = lutweights;  // Only Lutweights[0].xy is used

  o0 = ProcessLutbuilder(float3(untonemapped_ap1), s0_s, t0, cb_config, o0, asuint(cb0[31].w));

#ifndef LUTBUILDER_HASH
#define LUTBUILDER_HASH 0
#endif
...
cbuffer _RootShaderParameters : register(b0) {
  ...
  float expand_gamut : packoffset(c036.w);
  ...
  uint output_device : packoffset(c040.w);
  uint output_gamut : packoffset(c041.x);
};

// #endif

// Declare cbuffers as varliables
// Makes it easier to move things around

// #define UE_FILMBLACKCLIP    = FilmBlackClip;
// #define UE_FILMTOE          = FilmToe;
// #define UE_FILMSHOULDER     = FilmShoulder;
// #define UE_FILMSLOPE        = FilmSlope;
// #define UE_FILMWHITECLIP    = FilmWhiteClip;
// #define UE_TONECURVEAMMOUNT = ToneCurveAmount;
// #define UE_BlUECORRECTION   = BlueCorrection;

static float ue_filmblackclip = FilmBlackClip;
static float ue_filmtoe = FilmToe;
static float ue_filmshoulder = FilmShoulder;
static float ue_filmslope = FilmSlope;
static float ue_filmwhiteclip = FilmWhiteClip;
static float ue_tonecurveammount = ToneCurveAmount;
static float ue_bluecorrection = BlueCorrection;

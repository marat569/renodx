...
void main(
  ...
  if (cb0[44].y != 0) {  // if (bUseMobileTonemapper)
    ...
    if (injectedData.toneMapType != 0.f && is_hdr) {
      ...
      config.type = 2.f;  // Force to 2.f for ACES
  ...
  if (cb0[65].z == 0) {                                                           // cb0[65].z = output device
    ...
    if (r0.w != 0) {  // lut sample somewhere here? || tonemap output?
